...
**********************************************
	...
	usage:
			riji.php?act=add&content=xxx	添加一条
				返回：{error:0, id: 新添加内容的ID, time: 添加时间}
			
			riji.php?act=get_page_count	获取页数
				返回：{count:页数}
			
			riji.php?act=get&page=1		获取一页数据
				返回：[{id: ID, content: "内容", time: 时间戳, acc: 顶次数, ref: 踩次数}, {...}, ...]
			
			riji.php?act=acc&id=12			顶某一条数据
				返回：{error:0}
			
			riji.php?act=ref&id=12			踩某一条数据
				返回：{error:0}
...
switch($act)
{
	case 'add':
		...
		$sql="INSERT INTO riji (ID, content, time, acc, ref) VALUES(0, '{$content}', {$time}, 0, 0)";
		...
		break;
	case 'get_page_count':
		$sql="SELECT COUNT(*)/{$PAGE_SIZE}+1 FROM riji";
	...
	case 'get':  //获得一页数据
		...
		$sql="SELECT ID, content, time, acc, ref FROM riji ORDER BY time DESC LIMIT {$s}, {$PAGE_SIZE}";
	...
	case 'ID':
        ...
        $sql = "DELETE FROM riji WHERE ID = {$id}";
